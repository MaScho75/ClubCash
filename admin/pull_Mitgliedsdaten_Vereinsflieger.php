    ...
    if ($restInterface->GetUsers()) {
        ...
        $filteredUsers = array_filter(array_map(function($user) use ($config) {
            if (!empty($user['key2designation'])) {
                ...
                // Fehleranalyse für roles
                echo "<h2>Debugging-Output</h2>";
        
                if (isset($user['roles'])) {
                    echo "<p>Rohdaten: <pre>" . htmlspecialchars(print_r($user['roles'], true)) . "</pre></p>";
                } else {
                    echo "<p><strong>Kein 'roles' Daten vorhanden</strong></p>";
                }
        
                if (!empty($roles)) {
                    echo "<p>Dekodiert: <pre>" . htmlspecialchars(print_r($roles, true)) . "</pre></p>";
                } else {
                    echo "<p><strong>Keine dekodierten Rollen vorhanden</strong></p>";
                }
        ...
        echo "<p>❌ Fehler beim Abrufen der Daten aus Vereinsflieger.de. Status Code: " . $restInterface->HttpStatusCode . "</p>\n";
    }
} else {
